...
class Client {
    ...
    handleIncoming(header, data) {
        ...
        if (header === Enums_1.ServerBound.Init) {
            ...
            // Finish handshake
            this.write().u8(Enums_1.ClientBound.Accept).vi(this.accessLevel).send();
        ...
        switch (header) {
            ...
            case Enums_1.ServerBound.Input: {
                ...
                if (this.inputs.isPossessing && this.accessLevel !== 3 /* config.AccessLevel.FullAccess */)
                    return;
                if ((flags & Enums_1.InputFlags.godmode)) {
                    if (this.accessLevel >= 2 /* config.AccessLevel.BetaAccess */) {
                    ...
                    else if (this.game.arena.arena.values.GUI & Enums_1.GUIFlags.canUseCheats) {
                        // TODO:
                        // Make it real invincibility
                        if (!player.spawnProtectionEnded)
                            player.spawnProtectionEnded = true;
                        else
                            player.style.styleFlags ^= Enums_1.StyleFlags.invincibility;
                    }
                }
                ...
                if ((flags & Enums_1.InputFlags.switchtank) && !(previousFlags & Enums_1.InputFlags.switchtank)) {
                    if (this.accessLevel >= 2 /* config.AccessLevel.BetaAccess */ || (this.game.arena.arena.values.GUI & Enums_1.GUIFlags.canUseCheats)) {
                        player.name.nametag |= Enums_1.NametagFlags.cheats;
                        this.devCheatsUsed = 1;
                ...
                if (flags & Enums_1.InputFlags.levelup) {
                    ...
                    if ((this.accessLevel === 3 /* config.AccessLevel.FullAccess */) || (camera.camera.values.level < 45 && ((this.game.arena.arena.values.GUI & Enums_1.GUIFlags.canUseCheats) || (this.accessLevel === 2 /* config.AccessLevel.BetaAccess */)))) {
                        player.name.nametag |= Enums_1.NametagFlags.cheats;
                        this.devCheatsUsed = 1;
                ...
                */
                ...
                if (!this.inputs.isPossessing) {
                    ...
                    for (let i = 0; i < AIs.length; ++i) {
                        if ((AIs[i].state !== AI_1.AIState.possessed) && ((AIs[i].owner.relations.values.team === camera.relations.values.team && AIs[i].isClaimable) || this.accessLevel === 3 /* config.AccessLevel.FullAccess */)) {
    ...
    possess(ai) {
        if (!this.camera?.camera || ai.state === AI_1.AIState.possessed)
            return false;
        this.inputs.deleted = true;
        ...
        if (this.camera?.camera.values.player instanceof Object_1.default)
            this.camera.camera.values.player.state |= this.camera.camera.values.player.style.state.color = 1;
        ...
        for (let i = 0; i < Enums_1.StatCount; ++i)
            this.camera.camera.statLevels[i] = 0;
        for (let i = 0; i < Enums_1.StatCount; ++i)
            this.camera.camera.statLimits[i] = 7;
        for (let i = 0; i < Enums_1.StatCount; ++i)
            this.camera.camera.statNames[i] = "";
        ...
        if (ai.owner instanceof TankBody_1.default) {
            ...
            for (let i = 0; i < Enums_1.StatCount; ++i)
                this.camera.camera.statLevels[i] = ai.owner.cameraEntity.camera.statLevels.values[i];
            for (let i = 0; i < Enums_1.StatCount; ++i)
                this.camera.camera.statLimits[i] = ai.owner.cameraEntity.camera.statLimits.values[i];
            for (let i = 0; i < Enums_1.StatCount; ++i)
                this.camera.camera.statNames[i] = ai.owner.cameraEntity.camera.statNames.values[i];
    ...
    tick(tick) {
        ...
        if (!this.camera) {
            ...
            this.camera.camera.camera &= ~Enums_1.CameraFlags.showDeathStats;
        }
